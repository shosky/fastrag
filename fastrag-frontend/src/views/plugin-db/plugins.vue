...
async function loadData() {
  loading.value = true
  try {
    const res = (api as any).getPluginList({ page: currentPage.value, pageSize: pageSize.value, keyword: searchKeyword.value || undefined })
...
async function handleDelete(row: any) {
  try {
    ...
    const fn = (api as any)['deletePlugins'] || (api as any).deleteFaq
    if (fn) await fn(row.id)
    ElMessage.success('删除成功'); loadData()
  } catch {}
}
async function handleSave() {
    if (!formData.value.name) { ElMessage.warning('请输入插件名称'); return }
  try {
    const cfn = (api as any)['createPlugins'] || (api as any).createFaq
    const ufn = (api as any)['updatePlugins'] || (api as any).updateFaq
  ...
  <div class="page-container" v-loading="loading">
    <div class="card-panel">
      ...
      <el-table :data="dataList" stripe>
          <el-table-column prop="name" label="插件名称" min-width="200" show-overflow-tooltip />
      ...
      <el-form label-width="100px">
            ...
            <el-form-item label="类型" >
              <el-select v-model="formData.type" placeholder="请选择类型" style="width: 100%">
                <el-option v-for="opt in [{label:'自定义',value:'custom'},{label:'上传',value:'uploaded'},{label:'JSON导入',value:'json_import'}]" :key="opt.value" :label="opt.label" :value="opt.value" />
              </el-select>
            </el-form-item>
      </el-form>
      <template #footer>
