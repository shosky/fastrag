...
public class ParseStrategyServiceImpl implements ParseStrategyService {
    ...
    @Override public ParseStrategyDto create(String kbId,ParseStrategyRequest req) { var e=new KbParseStrategy(); e.setKbId(kbId); e.setName(req.getName()); e.setDescription(req.getDescription()); e.setExtensions(req.getExtensions()!=null?JSONUtil.toJsonStr(req.getExtensions()):null); e.setParseMethod(req.getParseMethod()); e.setIsDefault(0); e.setAdvanced(req.getAdvanced()!=null?JSONUtil.toJsonStr(req.getAdvanced()):null); e.setLlmModel(req.getLlmModel()); e.setVlmModel(req.getVlmModel()); mapper.insert(e); return toDto(e); }
    @Override public ParseStrategyDto update(String kbId,String id,ParseStrategyRequest req) { var e=getOne(kbId,id); if(req.getName()!=null)e.setName(req.getName()); if(req.getDescription()!=null)e.setDescription(req.getDescription()); if(req.getExtensions()!=null)e.setExtensions(JSONUtil.toJsonStr(req.getExtensions())); if(req.getParseMethod()!=null)e.setParseMethod(req.getParseMethod()); if(req.getLlmModel()!=null)e.setLlmModel(req.getLlmModel()); if(req.getVlmModel()!=null)e.setVlmModel(req.getVlmModel()); if(req.getAdvanced()!=null)e.setAdvanced(JSONUtil.toJsonStr(req.getAdvanced())); mapper.updateById(e); return toDto(e); }
