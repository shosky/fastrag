...
public class ParseStrategyServiceImpl implements ParseStrategyService {
    ...
    public List<ParseStrategyDto> list(String kbId) {
        return mapper.selectList(new LambdaQueryWrapper<KbParseStrategy>().eq(KbParseStrategy::getKbId, kbId))
                .stream().map(this::toDto).collect(Collectors.toList());
    }

    @Override
    public ParseStrategyDto get(String kbId, String id) {
        return toDto(getOne(kbId, id));
    }
    ...
    public ParseStrategyDto create(String kbId, ParseStrategyRequest req) {
        ...
        e.setVlmModel(req.getVlmModel());
        e.setEnableGraphBuild(req.getEnableGraphBuild() != null && req.getEnableGraphBuild() ? 1 : 0);
        mapper.insert(e);
        return toDto(e);
    }
    ...
    public ParseStrategyDto update(String kbId, String id, ParseStrategyRequest req) {
        ...
        if (req.getVlmModel() != null) e.setVlmModel(req.getVlmModel());
        if (req.getAdvanced() != null) e.setAdvanced(JSONUtil.toJsonStr(req.getAdvanced()));
        if (req.getEnableGraphBuild() != null) e.setEnableGraphBuild(req.getEnableGraphBuild() ? 1 : 0);
        mapper.updateById(e);
        return toDto(e);
    }
    ...
    public void delete(String kbId, String id) {
        var e = getOne(kbId, id);
        if (e.getIsDefault() == 1) throw new RuntimeException("Cannot delete default");
        mapper.deleteById(id);
    }

    @Override
    public void setDefault(String kbId, String id) {
        mapper.selectList(new LambdaQueryWrapper<KbParseStrategy>().eq(KbParseStrategy::getKbId, kbId).eq(KbParseStrategy::getIsDefault, 1))
    ...
    public ParseStrategyDto resolveByExtension(String kbId, String ext) {
        var all = mapper.selectList(new LambdaQueryWrapper<KbParseStrategy>().eq(KbParseStrategy::getKbId, kbId));
        for (var s : all) {
    ...
    private KbParseStrategy getOne(String kbId, String id) {
        var e = mapper.selectOne(new LambdaQueryWrapper<KbParseStrategy>().eq(KbParseStrategy::getKbId, kbId).eq(KbParseStrategy::getId, id));
    ...
    private ParseStrategyDto toDto(KbParseStrategy e) {
        ...
        d.setVlmModel(e.getVlmModel());
        d.setEnableGraphBuild(e.getEnableGraphBuild());
        return d;
    }
}
