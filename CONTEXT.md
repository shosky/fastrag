...
**知识库（Knowledge Base）** — 用户创建的知识管理单元，包含文件、分块、QA 对、图谱等子资源。

**知识分块（Chunk）** — 文档经过解析和切分后的文本片段，是向量检索和图谱构建的最小单元。

**解析策略（Parse Strategy）** — 定义文档切分规则的配置，包含分块长度、重叠、分隔符、LLM/VLM 模型等参数。

**向量检索（Vector Retrieval）** — 通过文本向量化后在 Milvus 中进行相似度搜索的检索方式。

**混合检索（Hybrid Retrieval）** — 向量检索 + 关键词检索 + 重排序（Rerank）组合的检索策略。

**知识图谱（Knowledge Graph）** — 从分块中抽取实体和关系构建的图结构，存储在 Neo4j 或 MySQL Graph Store 中。
